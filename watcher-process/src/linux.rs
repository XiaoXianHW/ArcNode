...
pub fn start_monitoring(device_id: String, storage: Arc<Storage>, running: Arc<AtomicBool>) -> Result<()> {
    ...
    info!("Starting Linux process monitor");
...
fn monitor_processes(storage: Arc<Storage>, running: Arc<AtomicBool>) {
    ...
    let initial_processes: HashMap<u32, String> = sys.processes()
        .iter()
        .filter(|(_, proc)| has_display_variable(proc))
...
fn has_display_variable(process: &sysinfo::Process) -> bool {
    use std::fs;
    let pid = process.pid().as_u32();
    if let Ok(environ) = fs::read_to_string(format!("/proc/{}/environ", pid)) {
        return environ.contains("DISPLAY=") || environ.contains("WAYLAND_DISPLAY=");
    }
    false
}
